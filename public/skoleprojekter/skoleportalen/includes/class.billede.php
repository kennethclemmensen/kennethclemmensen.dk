...
class billede{
	...
	/**
	 * Sætter egenskaben billede. Modtager $_FILES['billede'] eller tilsvarende array og returnerer navnet.
	 *
	 * @param array $files
	 */
	public function setBillede($files){
		$this->billede = $files;
		$this->navn = strtolower($files['name']);
		return time()."_".$this->navn;
	}
	...
	public function delete($navn){
		if (file_exists($this->mappe . $navn)){
			unlink($this->mappe . $navn);
		}
		$original = ltrim($navn, "thumb_");
		if (file_exists($this->mappe . $original)) {
